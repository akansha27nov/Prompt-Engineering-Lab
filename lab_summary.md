...
Sentiment improved from 20% to 93% consistency, with the underlying label itself never wrong across five diverse test messages including sarcasm — only a cosmetic "Sentiment:" prefix remains unresolved.

Extraction improved from 20% to 100% consistency on the base case and held 100% JSON validity across all four stress-test variations, though a genuine schema gap was found: messages with multiple orders break the single-order assumption.
